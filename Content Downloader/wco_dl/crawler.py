...
class Crawler:
    ...
    if __name__ == '__main__':
        ...
        parser.add_argument('-epr', '--episoderange', nargs=1, help='Specifies the range of episodes to download.', default='All')
        parser.add_argument('-se', '--season', nargs=1, help='Specifies the season to download.', default='All')
        parser.add_argument('-x', '--exclude', nargs=1, help='Specifies the episodes to not download (ie ova).', default=None)
        ...
        parser.add_argument("-q", "--quiet", help="Will not show download progress", action="store_true")
        logger = "False"
        quiet = 'False'        
        ...
        if args.batch:
            ...
            if (type(args.threads) == list): args.threads = args.threads[0]
            ...
            with open(args.batch[0], 'r') as anime_list:
                ...
                for anime in anime_list:
                    ...
                    # print(anime.replace('\n', ''))
                    Lifter(url=anime.replace('\n', '').replace('https://wcostream.com', 'https://www.wcostream.com'), resolution=args.highdef, logger=logger, season=args.season, ep_range=args.episoderange, exclude=args.exclude, output=args.output, newest=args.newest, settings=settings, database=database, threads=args.threads, quiet=quiet)
        ...
        if args.update_shows: 
            ...
            for x in database.return_show_url():
                ...
                Lifter(url=x, resolution=args.highdef, logger=logger, season=args.season, ep_range=args.episoderange, exclude=args.exclude, output=args.output, newest=args.newest, settings=settings, database=database, update=True, quiet=quiet)
        ...
        if args.verbose:
            ...
            logging.debug("Operating System : {0} - {1} - {2}".format(platform.system(), platform.release(), platform.version()))
        ...
        if args.highdef: args.highdef = '720'
        else: args.highdef = '480'
        
        if args.input is None:

            # print("Please enter the required argument. Run __main__.py --help")
            print("Please enter the required argument. Run crawler.py --help")
            exit()
            ...
            if (type(args.episoderange) == list):
                ...
                if '-' in args.episoderange[0]: args.episoderange = args.episoderange[0].split('-')
                else: args.episoderange = args.episoderange[0]

            if (type(args.season) == list): args.season = args.season[0]
            if (type(args.output) == list): args.output = args.output[0]

            if (type(args.exclude) == list):

                if ',' in args.exclude[0]: args.exclude = args.exclude[0].split(',')
                else: args.exclude = args.exclude[0]

            if (type(args.threads) == list): args.threads = args.threads[0]
            Lifter(url=args.input[0].replace('https://wcostream.com', 'https://www.wcostream.com'), resolution=args.highdef, logger=logger, season=args.season, ep_range=args.episoderange, exclude=args.exclude, output=args.output, newest=args.newest, settings=settings, database=database, threads=args.threads, quiet=quiet)