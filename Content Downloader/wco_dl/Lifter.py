...
class Lifter(object):
    ...
    def check_output(self, anime_name):
        ...
        else: output_directory = self.output.translate(str.maketrans({'\\': os.sep, '/': os.sep}))
        return output_directory

    def request_c(self, url, extraHeaders=None):

        myheaders = {'User-Agent': self.user_agent,
        ...
        if extraHeaders:myheaders.update(extraHeaders)
        cookie = None
    ...
    def download_show(self, url):
        ...
        if season != "season-All" and ep_range != "All":
            ...
            episodes = ["episode-{0}".format(n) for n in range(int(ep_range[0]), int(ep_range[1]) + 1)]
    ...
    def info_extractor(url):
        ...
        try:
            ...
            if "season" in url:
                ...
                show_name, season, episode, desc = re.findall(r'([a-zA-Z0-9].+)\s(season\s\d+\s?)(episode\s\d+\s)?(.+)', url.replace('-', ' '))[0]

            else:

                show_name, episode, desc = re.findall(r'([a-zA-Z0-9].+)\s(episode\s\d+\s?)(.+)', url.replace('-', ' '))[0]
                season = "season 1"
