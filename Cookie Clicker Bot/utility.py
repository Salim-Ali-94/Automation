...
def loadCheckpoint(browser, shop, opened = True):
	...
	browser.execute_script("arguments[0].click();", options)
	browser, stats = getStats(browser)
	print(f"Loaded latest checkpoint for {shop} with the following stats:\n")
	for stat in stats: print(f"{stat}: {stats[stat]}")
	print()
	return browser
...
def getStats(browser):
	...
	collect[statistics[0].text.split(":", 1)[0]] = statistics[0].find_element_by_css_selector(".price.plain").text.lstrip().rstrip()
	collect[statistics[1].text.split(":", 1)[0]] = statistics[1].find_element_by_css_selector(".price.plain").text.lstrip().rstrip()
	collect[statistics[2].text.split(":", 1)[0]] = statistics[2].find_element_by_css_selector(".price.plain").text.lstrip().rstrip()
	collect[statistics[3].text.split(":", 1)[0]] = " ".join(statistics[3].text.split(":", 1)[1:]).lstrip().rstrip()
	collect[statistics[4].text.split(":", 1)[0]] = " ".join(statistics[4].text.split(":", 1)[1:]).lstrip().rstrip()
	collect[statistics[5].text.split(":", 1)[0]] = " ".join(statistics[5].text.split(":", 1)[1:]).lstrip().rstrip()
	collect[statistics[6].text.split(":", 1)[0]] = " ".join(statistics[6].text.split(":", 1)[1:]).lstrip().rstrip()
	collect[statistics[7].text.split(":", 1)[0]] = " ".join(statistics[7].text.split(":", 1)[1:]).lstrip().rstrip()
	collect[statistics[8].text.split(":", 1)[0]] = " ".join(statistics[8].text.split(":", 1)[1:]).lstrip().rstrip()
	collect[statistics[9].text.split(":", 1)[0]] = " ".join(statistics[9].text.split(":", 1)[1:]).lstrip().rstrip()
	collect[statistics[10].text.split(":", 1)[0]] = " ".join(statistics[10].text.split(":", 1)[1:]).lstrip().rstrip()
	stats = browser.find_element_by_id("statsButton")
	browser.execute_script("arguments[0].click();", stats)
	return browser, collect
